...
void do_kill (dbref player, const char *what, int cost)
{
  ...
  default:
    if (Typeof (victim) != TYPE_PLAYER) {
      ...
      if (!payfor (player, cost)) {
      ...
      } else if ((RAND () % KILL_BASE_COST) < cost && !Wizard (victim)) {
        /* you killed him */
        ...
        if (db[victim].pennies < MAX_PENNIES) {
          sprintf (buf, "Your insurance policy pays %d pennies.", KILL_BONUS);
          notify (victim, buf);
...
void do_give (dbref player, char *recipient, int amount)
{
  ...
  if (!payfor (player, amount)) {
    ...
    sprintf (buf, "You give %d %s to %s.",
      amount, amount == 1 ? "penny" : "pennies", db[who].name);
    notify (player, buf);
    if (Typeof (who) == TYPE_PLAYER) {
      sprintf (buf, "%s gives you %d %s.",
        db[player].name, amount, amount == 1 ? "penny" : "pennies");
      notify (who, buf);
    }
