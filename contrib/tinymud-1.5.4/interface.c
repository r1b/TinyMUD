...
const char *addrout (struct in_addr);
...
void set_signals (void)
{
  ...
  signal (SIGUSR1, bailout);
#ifdef DETACH
  /* SIGUSR2 synchronizes the log file */
  signal (SIGUSR2, logsynch);
#else   /* DETACH */
...
struct descriptor_data *new_connection (int sock)
{
  ...
  if (newsock == INVALID_SOCKET) {
    ...
    strcpy (hostname, addrout (addr.sin_addr));
#ifdef NOISY_LOG
...
{
  send (d->descriptor, LEAVE_MESSAGE, strlen (LEAVE_MESSAGE), 0);
}
  ...
  while (d) {
    if (d->connected &&
      ...
      if (tabular) {
        ...
        if (wizard)
          sprintf (buf + strlen (buf), " %s%s", flagbuf, d->hostname);
      } else {
        sprintf (buf,
          "%s idle %ld seconds", db[d->player].name, now - d->last_time);
        if (wizard)
          sprintf (buf + strlen (buf), " %sfrom host %s", flagbuf, d->hostname);
      }
      strcat (buf, "\n");
      queue_string (e, buf);
    }
